...
export function QueryNullable(key, item){
  return New_1((path) => {
    ...
    if(m!=null&&m.$==1){
      ...
      return map((_1) =>[Route.New(path.Segments, newQa, path.FormData, path.Method, path.Body), _1[1]], item.Parse(Route.New(ofArray([q]), Route.Empty.QueryArgs, Route.Empty.FormData, Route.Empty.Method, Route.Empty.Body)));
    }
...
export function QueryOption(key, item){
  return New_1((path) => {
    ...
    if(m!=null&&m.$==1){
      const q=m.$0;
      const newQa=path.QueryArgs.Remove_1(key);
      return map((_1) =>[Route.New(path.Segments, newQa, path.FormData, path.Method, path.Body), Some(_1[1])], item.Parse(Route.New(ofArray([q]), Route.Empty.QueryArgs, Route.Empty.FormData, Route.Empty.Method, Route.Empty.Body)));
    }
    else return[[path, null]];
  }, (value) => {
    let _1;
